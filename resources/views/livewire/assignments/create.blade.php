        ...
        <div class="fixed inset-0 flex items-center justify-center z-50">
            <div class="bg-white p-6 rounded-xl w-[450px] max-h-[90vh] overflow-y-auto shadow-xl space-y-4">
                ...
                <div class="flex justify-between items-center mb-2">
                    <h2 class="text-lg font-semibold">Tambah Tugas Pribadi</h2>
                ...
                <form wire:submit.prevent="store" class="space-y-4">
                    ...
                    {{-- Kategori (custom dropdown mirip group) --}}
                    <label class="font-medium">Kategori</label>
                    <div class="relative" x-data="{ open: false }">
                        <button
                            type="button"
                            @click="open = !open"
                            class="w-full border px-4 pr-8 py-2.5 rounded-xl flex justify-between items-center bg-white hover:bg-gray-50"
                        >
                            <div class="flex items-center gap-2 flex-1 min-w-0">
                                @if($selectedCategoryIcon)
                                    <img src="{{ asset('img/categoryIcons/' . $selectedCategoryIcon) }}" class="w-5 h-5">
                                @endif
                                <span class="truncate {{ $selectedCategoryName ? 'text-gray-900' : 'text-gray-500' }}">{{ $selectedCategoryName ?? 'Pilih Kategori' }}</span>
                            </div>

                            <svg class="w-5 h-5 absolute right-3 top-1/2 -translate-y-1/2 text-gray-600 flex-shrink-0" fill="none" stroke="currentColor">
                        ...
                        <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
                            <div class="bg-white p-6 rounded-xl w-[350px] shadow-xl">
                                ...
                                <div class="space-y-4">
                                    ...
                                    />
                                    <div>
                                        <p class="font-medium mb-2">Pilih Icon (opsional)</p>
                    ...
                    <div class="flex gap-1 w-full">
                        <div class="w-[69%]">
                            <flux:input
                                ...
                                icon="calendar"
                                type="date"
                                wire:model.defer="due_date_date"
                            />
                        </div>
                        ...
                        <div class="w-[31%]">
                            <flux:input
                                ...
                                icon="clock"
                                type="time"
                                wire:model.defer="due_date_time"
                                step="60"
                            />
                        </div>
                    </div>
                    @error('due_date_date') <div class="text-sm text-red-600">{{ $message }}</div> @enderror
                    @error('due_date_time') <div class="text-sm text-red-600">{{ $message }}</div> @enderror

                    {{-- Prioritas --}}
                    <flux:select placeholder="Seberapa Penting Tugasmu?" :label="__('Prioritas')" wire:model.defer="priority_id">
                    ...
                    @error('priority_id') <div class="text-sm text-red-600">{{ $message }}</div> @enderror

                    {{-- Tipe tugas (custom dropdown mirip group) --}}
                    ...
                    <div class="relative" x-data="{ open: false }">
                        ...
                        >
                            <div class="flex items-center gap-2 flex-1 min-w-0">
                                ...
                                @endif
                                <span class="truncate {{ $selectedTypeName ? 'text-gray-900' : 'text-gray-500' }}">{{ $selectedTypeName ?? '-- Pilih Tipe --' }}</span>
                            </div>
                    ...
                    @error('type_assignment_id') <div class="text-sm text-red-600">{{ $message }}</div> @enderror

                    {{-- Modal tambah tipe --}}
                    @if($showTypeModal ?? false)
                        <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
                            <div class="bg-white p-6 rounded-xl w-[350px] shadow-xl">
                                <div class="flex justify-between mb-3">
                                ...
                                <div class="space-y-4">
                                    <input
                                        type="text"
                                        wire:model.defer="newTypeName"
                                        class="w-full border px-3 py-2 rounded-lg"
                                        placeholder="Nama tipe tugas"
                                    />
                                    @error('newTypeName') <div class="text-sm text-red-600">{{ $message }}</div> @enderror
                                    ...
                                    <input
                                        ...
                                        min="0"
                                        max="255"
                                        wire:model.defer="newTypeColor"
                                        class="grayscale-range w-full cursor-pointer"
                                    >
                                    @error('newTypeColor') <div class="text-sm text-red-600">{{ $message }}</div> @enderror
